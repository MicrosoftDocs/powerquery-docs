...
> [!Note]
> If you implement your own OAuth flow for Azure AD, users who have enabled [Conditional Access](https://docs.microsoft.com/en-us/azure/active-directory/conditional-access/overview) for their tenant may encounter issues when refreshing via the Power BI service. This would not impact Gateway based refresh, but would impact a certified connector that supports refresh from the Power BI service. Users may run into a problem stemming from the connector using a [public client application](https://docs.microsoft.com/en-us/azure/active-directory/develop/msal-client-applications) when configuring web based credentials through the Power BI service. The access token generated by this flow will ultimately be used on a different machine (i.e. the power bi service in an Azure data center, not on the company's network) than the one used to originally authenticate (i.e. the machine of the user who configures the data source credentials, on the company's network). The built-in `Aad` type works around this problem by using a different Azure AD client when configuring credentials in the Power BI service. This option will not be available to connectors that use the `OAuth` authentication kind.
