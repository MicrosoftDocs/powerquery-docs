...
> [!NOTE]
> The passphrase input field will automatically appear only if required, based on the type of certificate uploaded. After the user uploads a certificate, the system detects whether it is encrypted or unencrypted and conditionally displays the passphrase dialog. Both encrypted and unencrypted keys are supported.
