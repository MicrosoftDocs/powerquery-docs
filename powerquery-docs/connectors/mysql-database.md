...
| Supported authentication types | Windows (Power BI Desktop, Excel, online service with gateway)<br/>Database (Power BI Desktop, Excel)<br/>Basic (online service with and without gateway) |
...
1. Select the **MySQL database** option in the connector selection. For more information, see [Where to get data](../where-to-get-data.md).

1. In the **MySQL database** dialog, enter the name of the server and database.

   :::image type="content" source="./media/mysql-database/signin.png" alt-text="Screenshot of the MySQL database connection dialog.":::

1. Select the **Database** authentication type and enter your MySQL credentials in the **User name** and **Password** boxes.
...
1. Select the level to apply your credentials to.

1. When you finish, select **OK**.

1. If the connection isn't encrypted, you see the following dialog.
...
| --------------- | ----------- |
| Command timeout in minutes | If your connection lasts longer than 10 minutes (the default timeout), enter another value in minutes to keep the connection open longer. |
| SQL statement | For information, see [Import data from a database using native database query](../native-database-query.md). |
| Include relationship columns | If checked, includes columns that might have relationships to other tables. If you clear this box, those columns aren't included. |
| Navigate using full hierarchy | If checked, the navigator displays the complete hierarchy of tables in the database you're connecting to. If cleared, the navigator displays only the tables whose columns and rows contain data. |

When you select the advanced options you require, select **OK** in Power Query Desktop or **Next** in Power Query Online to connect to your MySQL database.
