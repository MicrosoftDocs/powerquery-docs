---
title: Include file for the Connect to Power Query Online section of the Power Query Folder connector documentation
description: Include file for the Connect to Power Query Online section of the Power Query Folder connector documentation
ms.date: 03/06/2026
ms.topic: include
---

To connect to a folder from Power Query Online:

1. Select the **Folder** option in the connector selection.

2. Enter the path to the folder you want to load.

   :::image type="content" source="/power-query/connectors/media/folder/folder-browse-online.png" alt-text="Screenshot of the connection settings for folder selection online.":::

3. Enter the name of an on-premises data gateway that you'll use to access the folder.

4. Select the authentication kind to connect to the folder. If you select the **Windows** authentication kind, enter your credentials.

5. Select **Next**.

6. In the **Navigator** dialog box, select **Combine** to combine the data in the files of the selected folder and load the data into the Power Query Editor for editing. Or select **Transform data** to load the folder data as-is in the Power Query Editor.

   :::image type="content" source="/power-query/connectors/media/folder/navigator-online.png" alt-text="Screenshot of the folder data open in the nagivator diaglog box.":::

