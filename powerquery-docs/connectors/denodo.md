...
2. There are two ways to connect to the data source of your choice:
   ...
   >
   > Doing otherwise could prevent Power BI from recognizing different Denodo semantic models in a report as belonging to the same Denodo data source and, as a consequence, request separate authentication credentials for each of them.
...
4. The third section, **Native Query**, is an optional field where you can enter a query. If this query field is used, the resulting semantic model will be the result of the query instead of a table or a set of tables.
