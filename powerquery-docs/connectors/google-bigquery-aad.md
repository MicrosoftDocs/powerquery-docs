...
| Audience Uri | `//iam.googleapis.com/locations/global/workforcePools/<workforce pool ID>/providers/azuread`<br/><br/>The *\<workforce pool ID>* must be changed to the workforce-pool name used when setting up the [OIDC configuration](#oidc-configurations). |
