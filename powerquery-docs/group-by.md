...
For this demonstration, you'll be using the sample table shown in the following image.
...
After removing your **Products** column and setting the data type for both newly expanded columns, your result looks like the one in the following image.

![Final table with all transformations](images/me-group-by-row-operation-final-table.png)

## Fuzzy grouping

To demonstrate how to do *Fuzzy grouping* in Power Query's *Group By* functionality, the sample table shown in the following image will be used.

![Fuzzy grouping sample source table](images/me-fuzzy-grouping-sample-source-table.png)

The goal of the *Fuzzy grouping* is to do a *Group by* operation using an approximate match algorithm for text strings. Power Query uses the Jaccard Similarity algorithm to measure the similarity between pairs of instances. Then it applies Agglomerative Hierarchical Clustering to group instances together. The following image shows the output that you expect, where the table will be grouped by the **Person** column.

![Fuzzy grouping sample final table no transform table](images/me-fuzzy-grouping-sample-final-table-no-transform-table.png)

To do the fuzzy grouping, you perform the same steps previously described in this article. The only difference is that this time, in the **Group by** window, you'll select **Use fuzzy grouping**.

![Fuzzy grouping radio button in Group by window](images/me-fuzzy-grouping-button-group-by-window.png)

For each group of rows, Power Query will pick the most frequent instance as the “canonical” instance. If multiple instances have the same frequency, Power Query will pick the first one. After you select **OK** in the **Group by** window, you'll get the result that you were expecting.
...
Here's a list of the available options for the *Fuzzy grouping*:
* **Similarity threshold (optional)**&mdash;This option indicates how similar two values need to be in order to be grouped together. The minimum value of 0 will cause all values to be grouped together. The maximum value of 1 will only allow exact match of values to be grouped together. The default is 0.8.)
* **Ignore case**&mdash;Used to compare text strings, ignoring case considerations. Enabled by default.
* **Group by combining text parts**&mdash;The algorithm will try to combine text parts (such as "Micro" and "soft" into "Microsoft") in order to group values.
* **Transformation table (optional)**&mdash;Select a transformation table that will be used to map values (such as "MSFT" to "Microsoft") to group them together. The table should contain a **From** column and a **To** column.
...
* **To**&mdash;The text string used to replace the text string found.

The following image shows the transformation table that will be used.

![Fuzzy grouping Sample Transformation table](images/me-fuzzy-grouping-sample-transformation-table.png)

Going back to the **Group by** window and expanding **Fuzzy group options**, select the **Transformation table** query from the dropdown.
...
If your goal is to remove the duplicates from a table, you can also use the [Remove duplicates](working-with-duplicates.md) operation in Power Query.
