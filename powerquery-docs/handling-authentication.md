...
The `Aad` authentication kind is a specialized version of OAuth for Microsoft Entra ID. It uses the same Microsoft Entra ID client as the built-in Power Query connectors that support Organization Account authentication. More information can be found in the [Configuring Entra for Custom Connectors](configure-entra-for-connector.md) quickstart guide.
...
Requesting the [.default scope](/entra/identity-platform/scopes-oidc#the-default-scope) will return a token with all previously authorized scopes for the Power Query client application ID.
