...
| Aad                 | AuthorizationUri | `text` value or function that returns the Microsoft Entra ID authorization endpoint (example: `"https://login.microsoftonline.com/common/oauth2/authorize"`).<br/></br>Go to the [Microsoft Entra ID authentication](#azure-active-directory-authentication) section. |
|                     | Resource         | `text` value or function that returns the Microsoft Entra ID resource value for your service.                                                                              |
...
The `Aad` authentication kind is a specialized version of OAuth for Microsoft Entra ID. It uses the same Microsoft Entra ID client as the built-in Power Query connectors that support Organization Account authentication.
