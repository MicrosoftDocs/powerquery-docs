...
* If you add a new step to a fully folded query and the new step also folds, Power Query may send a new request to the data source, rather than using a cached version of the previous result. In practice, this can result in seemingly simple operations on a small amount of data taking longer to refresh in the preview than expected, due to the fact that Power Query is re-querying the data source rather than working off a local copy of the data.