...
:::image type="content" source="media/append-queries/append-queries-diagram.png" alt-text="Diagram showing the result of an append operation with null values in columns that don't exist in one of the original tables." lightbox="media/append-queries/append-queries-diagram.png":::
...
You can find the **Append queries** command on the **Home** tab in the **Combine** group. On the dropdown list menu, there are two options:
...
- **Store Sales**: Sales made through the company's physical locations.
  ...
  :::image type="content" source="media/append-queries/append-queries-sample-store-sales.png" alt-text="Screenshot of the sample store sales table with date, units, referrer, customer ID, and channel name (store) columns." lightbox="media/append-queries/append-queries-sample-store-sales.png":::
...
:::image type="content" source="media/append-queries/append-queries-sample-three-more-tables-window.png" alt-text="Screenshot of the append dialog using three or more tables mode." lightbox="media/append-queries/append-queries-sample-three-more-tables-window.png":::

After you select **OK**, a new query is created with all your tables appended.

:::image type="content" source="media/append-queries/append-queries-sample-three-more-tables-output.png" alt-text="Screenshot with combined common columns and data from the three tables, except null values in the online and wholesale rows of the Referrer column." lightbox="media/append-queries/append-queries-sample-three-more-tables-output.png":::
