...
* **Show similarity scores**: Show similarity scores between the input and the matches values after fuzzy matching.
...
You can go back to the **Merge** dialog box, and in **Fuzzy matching options** under **Number of matches (optional)**, enter **1**. Enable the **Show similarity scores** option and, under **Transformation table (optional)**, select **Transform Table** from the drop-down menu.
...
After you select **OK**, you can go to the Merge step and when you try to expand the column with table values you'll notice that besides the *Fruit* field you'll also see the *Similarity score field*.  Select both and expand them without adding a prefix.

![Table expand dialog for the Fruits column which contains the Fruit and Similarity score fields selected.](images/fuzzy-merge-expand-similarity-score.png "Table expand dialog for the Fruits column which contains the Fruit and Similarity score fields selected")

After expanding these two fields, you will see them in your table. Notice the values that you get for the similarity scores of each value which could help you with further transformations if needed to determine if you need to lower or raise your similarity threshold. 
...
For this example, the *Similarity score* serves only as an informational component and is not needed in the output of this query so you can remove it. Note how the example started with nine distinct values, but after the fuzzy merge, there are only four distinct values. 
