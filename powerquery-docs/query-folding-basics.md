...
:::image type="content" source="media/query-folding-basics/diagram-1.png" alt-text="Diagram that shows the path of a query evaluation from M script to the final output." lightbox="media/query-folding-basics/diagram-1.png":::
...
1. Power Query determines what data needs to be extracted from the data source and submits a request to the data source.
1. The data source responds to the request from Power Query by transferring the requested data to Power Query.
1. Power Query receives the incoming data from the data source and does any transformations using the Power Query engine if necessary.
1. The results derived from the previous point are loaded to a destination.
...
:::image type="content" source="media/query-folding-basics/diagram-2.png" alt-text="Diagram that shows the optimization process during query evaluation." lightbox="media/query-folding-basics/diagram-2.png":::
...
1. The Query folding mechanism submits metadata requests to the data source to determine the capabilities of the data source, table schemas, relationships between different tables at the data source, and more.
1. Based on the metadata received, the query folding mechanism determines what information to extract from the data source and what set of transformations need to happen inside the Power Query engine. It sends the instructions to two other components that take care of retrieving the data from the data source and transforming the incoming data in the Power Query engine if necessary.
1. Once the internal components of Power Query receive the instructions, Power Query sends a request to the data source using a data source query.
1. The data source receives the request from Power Query and transfers the data to the Power Query engine.
1. Once the data is inside Power Query, the transformation engine inside Power Query (also known as mashup engine) does the transformations that couldn't be folded back or offloaded to the data source.
1. The results derived from the previous point are loaded to a destination.
...
> [!NOTE]  
> The query folding mechanism is primarily available in connectors for structured data sources such as, but not limited to, [SQL Server](connectors/sql-server.md) and [OData Feed](connectors/odata-feed.md). During the optimization phase, the engine might sometimes reorder steps in the query.
>
