...
A user can connect to Dataverse using a custom SQL statement or a [native database query](native-database-query.md). While there is no user interface for this experience, users are able to enter the query using the Power Query Advanced Editor. In order to use a native database query, a **Database** must be specified as the Source. 
