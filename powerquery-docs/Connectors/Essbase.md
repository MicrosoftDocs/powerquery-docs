...
- Expanding the Advanced options area exposes four more options.

      The Command timeout in minutes option allows to set the maximum time a command is allowed to run before Power BI abandons the call. If the Command timeout is reached, Power BI may retry two more times before completely abandoning the call. This setting is helpful for querying large amount of data. The default value of the command timeout is 140 seconds.

      The MDX statement option allows to give a specific MDX statement to the Oracle Essbase server to execute. Normally, Power BI allows to interactively determine the measures and dimensions of the cube to return. However, by specifying the MDX statement means loading the result of that statement. When you specify the MDX statement, you must also provide the Server (e.g., essbaseserver-1) and Application (e.g., Sample) where the MDX statement is to run. Also, you may only use the MDX statement in conjunction with Data Connectivity mode set to Import. Note: The values specified for Server and Application are case-sensitive.

      In general, the MDX generated by SmartView or one accepted by Essbase Admin is not 100% compatible with PowerBI.

      PowerBI requires measures to be specified on 0 axis in MDX query. In addition, level numbering is reversed in XMLA. The least granular level is level 0, 1, etc. in XML, but the opposite in Essbase ‘directly’. Thus level numbers are used explicitly in the MDX query they need to be adjusted.

      

![Essbase conection authentication with command timeout](./media/essbase/advanced_option_command_timeout.png)

![Essbase conection authentication with MDX](./media/essbase/advanced_option_MDX.png)
...
1. Ensure the Essbase Analytic Provider Services (APS) server is configured correctly for the Provider Servers and Standalone Servers in the Essbase Administration Service (EAS) console. See [Registering Essbase and Cluster in the Admin Console](https://docs.oracle.com/cd/E57185_01/EPMDO/ch02s08.html)
...
Ensure the Essbase Analytic Provider Services (APS) server is configured correctly for the Provider Servers and Standalone Servers in the Essbase Administration Service (EAS) console. See [Registering Essbase and Cluster in the Admin Console](https://docs.oracle.com/cd/E57185_01/EPMDO/ch02s08.html).
