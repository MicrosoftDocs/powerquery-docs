...
When you are receiving an error code 429, it's possibly due to exceeding the limit of 2000 requests per minute. This error typically resolves by itself if you wait a minute or two after the cooldown period ended. This limit is in place to prevent Dataflows and other Power BI functionality from having a degraded performance. Consequences due to the continued high load on the service might result in additional degraded performance, so we ask users to significantly reduce the number of requests to less than 2000 (limit) or fix your script/model to this specific limit (2000) to efficiently mitigate impact and avoid further issues. It's advised to avoid nested joins that re-request Dataflow data; instead, stage data and perform merges within your Dataflow instead of your Dataset.
