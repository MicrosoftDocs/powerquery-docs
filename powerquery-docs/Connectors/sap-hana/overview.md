...
| CommandTimeout | A duration that controls how long the server-side query is allowed to run before it is cancelled. The default value is ten minutes. |
...
* The `SapHana.Database` function now supports connection and command timeouts. See [connect using advanced options](#connect-using-advanced-options) for more information.
