...
:::image type="content" source="media/connector-authentication/odata-authentication.png" alt-text="Screenshot of the authentication dialog for an OData feed in Power Query Desktop." lightbox="media/connector-authentication/odata-authentication.png":::
...
:::image type="content" source="media/connector-authentication/odata-online-authentication.png" alt-text="Screenshot of the Connect to data source windows for the OData connector in Power Query Online." lightbox="media/connector-authentication/odata-online-authentication.png":::
...
:::image type="content" source="media/connector-authentication/url-level-setting.png" alt-text="Screenshot of the authentication dialog with the level selection displayed and the level set to the default setting." lightbox="media/connector-authentication/url-level-setting.png":::
...
After you set the authentication method for a connector's specific address, you won't need to select the authentication method for that connector using that URL address or any subaddress again. For example, let's say you select the `https://contoso.com/` address as the level you want the Web connector URL settings to apply to. Whenever you use a [Web](connectors/web/web.md) to access any webpage that begins with this address, you aren't required to select the authentication method again.
...
1. Do one of the following:
   ...
   - In Power BI Desktop, on the **File** tab, select **Options and settings** > **Data source settings**.
     ...
     :::image type="content" source="media/connector-authentication/pbi-edit.png" alt-text="Screenshot of the Power BI Desktop File tab pane, with Options and settings and Data source settings emphasized." lightbox="media/connector-authentication/pbi-edit.png":::
...
1. In the **Data source settings** dialog box, select **Global permissions**, choose the data source where you want to change the permission setting, and then select **Edit Permissions**.

1. In the **Edit Permissions** dialog box, under **Credentials**, select **Edit**.
...
1. Change the credentials to the type required by the data source, select **Save**, and then select **OK**.
...
1. Enter the Northwind endpoint in the "Get Data" experience using the OData connector.
   ...
   :::image type="content" source="media/connector-authentication/northwind-odata.png" alt-text="Screenshot of the OData feed get data dialog with the Northwind site entered as the URL." lightbox="media/connector-authentication/northwind-odata.png":::

1. Select **OK** to enter the authentication experience. Normally, because Northwind isn't an authenticated service, you would just use **Anonymous**. To demonstrate lack of support for Microsoft Entra ID, choose **Organizational account**, and then select **Sign in**.

   :::image type="content" source="media/connector-authentication/northwind-auth.png" alt-text="Screenshot of the authentication dialog with the organizational account tab selected." lightbox="media/connector-authentication/northwind-auth.png":::

1. You encounter an error, indicating that OAuth or Microsoft Entra ID authentication isn't supported in the service.

   :::image type="content" source="media/connector-authentication/northwind-error.png" alt-text="Screenshot of the authentication dialog with the unable to connect error message displayed." lightbox="media/connector-authentication/northwind-error.png":::
...
1. Enter the URL in the "Get Data" experience using the OData connector.
   ...
   :::image type="content" source="media/connector-authentication/crm-auth.png" alt-text="Screenshot of the OData feed get data experience with the CRM address entered in the URL." lightbox="media/connector-authentication/crm-auth.png":::

1. Select **Organizational Account**, and then select **Sign-in** to proceed to connect using OAuth.

   :::image type="content" source="media/connector-authentication/crm-auth-dialog.png" alt-text="Screenshot of the authentication dialog with the organizational account selected and ready to sign in." lightbox="media/connector-authentication/crm-auth-dialog.png":::

1. The request succeeds and the OAuth flow continues to allow you to authenticate successfully.

   :::image type="content" source="media/connector-authentication/crm-auth-success.png" alt-text="Screenshot of the authentication dialog with the organizational account selected and the user successfully signed in." lightbox="media/connector-authentication/crm-auth-success.png":::
