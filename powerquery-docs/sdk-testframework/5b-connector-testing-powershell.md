...
> [!NOTE]
> Update the M query in the generated parameter query files to connect with your data source and retrieve the previously uploaded NycTaxiGreen & TaxiZoneLookup tables.

Alternatively, manually create the parameter query files and settings files for your data source by performing the below steps:

1. Navigate to the \testframework\tests\ConnectorConfigs folder under the cloned repo folder.
2. Make a copy of the \generic folder and rename it with the extension name.
3. Open each file in the \ParameterQueries folder and update the M query as the instructions provided in those files.
4. Open each file in the \Settings folder and update the correct parameter query file paths.
...
Alternatively, generate a credential template in JSON format for your connector using the **credential-template** command that can be passed into the **set-credential** command. For information on the usage to generate a credential template, refer to the [credential-template](/power-query/sdk-tools/pqtest-credentials#credential-template) section.
...
Then, store credentials using this **set-credential** command, which are used by the **compare** command to run the tests. Using the existing PowerShell window, set the credentials for your extension using the JSON credential file generated in the previous step. Refer to the [set-credential](/power-query/sdk-tools/pqtest-credentials#set-credential) section on the usage for setting up the credentials for your connector.
...
To run the complete set of the sanity and standard tests or a set of tests defined by the settings file, run the following command to execute the tests:
...
### Run using PQTest.exe
 ...
 To run particular tests directly using PQTest.exe, use the following command in the same PowerShell window:
