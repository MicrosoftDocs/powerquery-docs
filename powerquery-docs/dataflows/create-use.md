...
* [Incremental refresh](incremental-refresh.md) is limited to a maximum of 50 buckets (partitions) for a single query and 150 buckets for the whole dataflow. This limit is checked when you publish the dataflow. If a dataflow exceeds 150 buckets, the publish is blocked and you need to reduce the number of buckets (by increasing the bucket size or using incremental refresh on fewer queries) before you can publish again.
