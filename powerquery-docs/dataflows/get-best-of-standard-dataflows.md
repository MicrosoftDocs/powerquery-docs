...
* Mapping to Choice fields, configured with multiple select option enabled is only supported under these conditions: The dataflow only loads data to Choice fields with multiple select option enabled, and a comma separated list of values (integers) of the labels are used. For example, if the labels are "Choice1, Choice2, Choice3" with corresponding integer values of "1, 2, 3", then the column values should be "1,3" to select the first and last choices.
