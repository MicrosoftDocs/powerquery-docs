...
> [!NOTE]
> Standard V2 Dataflows rely on the createdon and modifiedon fields in order to remove rows that dont exist in the dataflows output, from the destination table. If those columns do not exist in the destination table, rows will not be deleted.
