...
There are a number of ways to work around this. Here are few suggestions:
...
* Use [ranking](/rank-column). For example, instead of using `Table.Distinct`, you could order by the column containing the duplicate values, rank based on a tie-breaker column (such as `modified_date`), and then filter to keep just the rank 1 rows.
